...
app.get("/services/:service", (req, res) => {
	pricing
		.describeServices({ ServiceCode: req.params.service, MaxResults: 20 })
		.promise()
		.then((data) => {
			res.status(200);
			res.json(data);
...
const describeAllServices = (services = [], next) => {
	return new Promise((resolve, reject) => {
		pricing
			.describeServices({ NextToken: next })
			.promise()
			.then((data) => {
				services = services.concat(data.Services);

				if (data.NextToken) {
					describeAllServices(services, data.NextToken).then((data) =>
						resolve(data)
					);
				} else {
					resolve(services);
...
const getAllProducts = (service, filters = [], products = [], next) => {
	let pricingFilters = Object.keys(filters).map((f) => {
